...
class MP_Artwork_Plugin_Gallery {
    ...
    private function set_post_gallery($src, $className, $id) {
        ...
        if ("gallery-12" != $className) {
            ...
            $image_medium = wp_get_attachment_image_src($id, mp_artwork_get_prefix() . 'thumb-medium');
            $image_thumbnails = wp_get_attachment_image_src($id, 'post-thumbnail');
            $html .= '<div class="' . $className . '"><a href="' . get_permalink() . '" ><img ';
            $html .= $this->get_img_attr($id, $src, mp_artwork_get_prefix() . 'thumb-large-blog');
    ...
    private function get_post_gallery_list($id_gallery, $size) {
        $src[0] = wp_get_attachment_image_src($id_gallery[0], mp_artwork_get_prefix() . 'thumb-large-blog');
        $className[0] = $this->get_post_gallery_class($src[0]);
        if ($size > 1) {
            $src[1] = wp_get_attachment_image_src($id_gallery[1], mp_artwork_get_prefix() . 'thumb-large-blog');
            $className[1] = $this->get_post_gallery_class($src[1]);
            if ($size > 2) {
                $src[2] = wp_get_attachment_image_src($id_gallery[2], mp_artwork_get_prefix() . 'thumb-large-blog');
    ...
    public function get_post_gallery($post, $mp_artwork_page_template) {
        if (get_post_gallery()) :
            ...
            $this->post_thumbnail($post, $mp_artwork_page_template);
        endif;
    }
    ...
    function post_thumbnail($post, $mp_artwork_page_template) {
        ?>
